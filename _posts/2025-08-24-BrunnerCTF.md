...
# Cooking Flags with BrunnerCTF 2025 - A Beginner’s Feast

Get your aprons on and terminals ready - the BrunnerCTF has just served up its very first edition, and I couldn’t resist grabbing a plate! 🍽️
...
In this post, I’ll walk you through my journey solving most of the beginner tasks across categories like misc, OSINT, web, crypto, forensics, boot2root, pwn, and reverse engineering. Along the way, I’ll share my thought process, lessons learned, and tips you can use if you’re just getting started with CTFs.
...
# Reverse the encoding: base 2 → base 10
...
Even though random.randrange(18) is called, there’s no seed specified. But importantly, the final key will always be a number between 0 and 255 because of how XOR works with bytes.

So, if we don’t know the key - brute-forcing all 256 possibilities is quick and easy.

## Brute-forcing the Key

Here’s the brute-force script I used:
```
cipher = "wg`{{pgna}&J{!x&2fJWg`{{&g;;;_!x&fJWg`{{&gh"
cipher_bytes = cipher.encode()

for key in range(256):
    decoded = ''.join(chr(b ^ key) for b in cipher_bytes)
    if "flag" in decoded or "{" in decoded:
        print(f"Key: {key} => {decoded}")
```

When running it, the readable candidate that stood out was:
```
Key 21: brunner{th3_n4m3's_Brunn3r...J4m3s_Brunn3r}
```
...
void vulnerable_dough_recipe() {
    ...
    gets(recipe);  // ⚠️ Dangerous! No length check
}
```
...
If we overwrite the return address to point to this function, we’ll get the flag.
...
- NX enabled → we can’t inject shellcode, but we can reuse code (return-to-function).
- No PIE → function addresses don’t change between runs.
...
- Position 0: b → b_ _ _ _
- Position 1: e → be_ _ _
- Positions 2-3: rr → berr_
- Position 4 (last char): must equal last char of word2 ("memory" → 'y') → berry
...
- words[0][:2] = First 2 chars of "red" → "re"
- words[1][:3] = First 3 chars of "memory" → "mem"
- words[2][:3] = First 3 chars of "berry" → "ber"
