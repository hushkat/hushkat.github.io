...
The Enduring Echo
HTB Sherlock Investigation write-up 
Challenge Overview
Difficulty: Easy
Category: Digital Forensics
What to expect: Windows Event Logs, Persistence, Lateral Movement, WMI
by hushkat
...
Scenario overview	3
Tools & Setup	3
1.	Create tools directory	3
2.	Download and extract Get-ZimmermanTools	3
3.	Install tools	3
Key Tools Used	4
Evidence Structure	4
What Are Logs? Definition	4
Investigation & Analysis	5
Event IDs Definition	5
Common Windows Event IDs (The Important Ones):	5
Initial Reconnaissance Command	5
Attack Chain Summary	15
Key Takeaways	15
...
$timestamp = (Get-Date).ToString("yyyyMMddHHmmss")
$password = "Watson_$timestamp"
By examining the account creation timestamp in Event ID 4720 and converting it to the appropriate timezone (UTC-7), the password becomes: Watson_20250824160509. Time-based password generation provides uniqueness while maintaining a predictable pattern for the attacker to reconstruct if needed. When investigating digital forensics, timestamps can be tricky because they might be stored in different timezones than when the actual event occurred.
